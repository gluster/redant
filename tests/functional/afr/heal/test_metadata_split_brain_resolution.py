...
class TestCase(DParentTest):
    ...
    def run_test(self, redant):
        ...
        cmd = (f"mkdir {self.mountpoint}/test_metadata_sb && "
               f"cd {self.mountpoint}/test_metadata_sb && "
               "for i in `seq 1 3`; do mkdir dir_$i; "
               "for j in `seq 1 5`; do dd if=/dev/urandom "
               "of=dir_$i/file_$j bs=1K count=1; done; "
               "dd if=/dev/urandom of=file_$i bs=1K count=1; done;")
        ...
        cmd = (f"cd {self.mountpoint}/test_metadata_sb && "
               "for i in `seq 1 2`; do chmod -R 0555 dir_$i file_$i;"
               "done;")
        redant.execute_abstract_op_node(cmd, self.client_list[0])
        ...
        cmd = (f"cd {self.mountpoint}/test_metadata_sb && "
               "for i in `seq 1 2` ; do chmod -R 0777 dir_$i file_$i;"
               "done;")
        redant.execute_abstract_op_node(cmd, self.client_list[0])
        ...
        cmd = (f"ls -lR {self.mountpoint}/test_metadata_sb")
        redant.execute_abstract_op_node(cmd, self.client_list[0])
