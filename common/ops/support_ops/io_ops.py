...
class IoOps(AbstractOps):
    ...
    def run_crefi(self, client: str, mountpoint: str, number: int,
                  breadth: int, depth: int, thread: int = 5,
                  random_size: bool = False, fop: str = 'create',
                  filetype: str = 'text', minfs: int = 10, maxfs: int = 500,
                  single: bool = False, multi: bool = False,
                  size: int = 100, interval: int = 100, nameBytes: int = 10,
                  random_filename: bool = True) -> bool:
        """
        ...
        Optional:
            ...
            fop(str): fop can be [create|rename|chmod|chown|chgrp|symlink
                      |hardlink|truncate|setxattr] this specifies the type
                      of fop to be executed by default it is create.
            filetype(str): filetype can be [text|sparse|binary|tar] this
                           specifie the type of file by default it is text.
            minfs(int): If random is set to true then this value has to be
                        altered to change minimum file size. (Value is in KB)
            maxfs(int): If random is set to true then this value has to be
                        altered to change maximum file size. (Value is in KB)
            ...
            random_filename(bool): It creates files with random names, if set
                                   to False it creates files with file name
        ...
        list_of_fops = ["create", "rename", "chmod", "chown", "chgrp",
                        "symlink", "hardlink", "truncate", "setxattr"]
        if fop not in list_of_fops:
            self.logger.error("fop value is not valid.")
            return False

        # Checking value of filetype.
        list_of_filetypes = ["text", "sparse", "binary", "tar"]
        if filetype not in list_of_filetypes:
            self.logger.error("filetype is not a valid file type.")
            return False

        # Checking if single and multi both are set to true.
        if single and multi:
            self.logger.error("single and mutli both can't be true.")
            return False

        # Checking if file size and random size arguments are given together.
        if (size > 100 or size < 100) and random_size:
            self.logger.error("Size and Random size can't be used together.")
            return False

        # Checking if minfs is greater than or equal to maxfs.
        if random_size and (minfs >= maxfs):
            self.logger.error("minfs shouldn't be greater than or equal to "
                              "maxfs.")
            return False

        # Creating basic command.
        command = f"crefi {mountpoint} -n {number} -b {breadth} -d {depth} "

        # Checking thread value and adding it, If greater or smaller than 5.
        if thread > 5 or thread < 5:
            command = f"{command} -T {thread}"
        ...
        if interval > 100 or interval < 100:
            command = f"{command} -I {interval}"

        # Checking if name Bytes is greater than or less than 10.
        if nameBytes > 10 or nameBytes < 10:
            command = f"{command} -l {nameBytes}"

        # Checking filetype and setting it if not
        # text.
        if filetype != "text":
            command = f"{command} -t {filetype}"

        # Running the command on the client node.
        ret = self.execute_abstract_op_node(command, client, False)
        if ret['error_code'] != 0:
            self.logger.error(f"Failed to run crefi on {client}")
            return False
